...
- **Reasoning layer** — **Claude** (`claude -p`) reads pre-digested text, interprets it against tracked axes, and writes journals + ontology deltas (`runner/single_pass_browse.js`, one schema-constrained call). Scoring, gating, and planning run on Claude too, via `runner/lib/compose.js`. Claude is the only LLM, with no fallback backend anywhere. The one exception — 0-3 relevance **scoring** on a local model via Ollama — was removed on 2026-08-25; scoring runs on Claude too, which makes it a ~90s subprocess call per candidate, so callers bound their concurrency. See INVENTORY §2.
