...
- **Reasoning layer** — **Claude** (`claude -p`) reads pre-digested text, interprets it against tracked axes, and writes journals + ontology deltas (`runner/single_pass_browse.js`, one schema-constrained call). Scoring, gating, and planning run on Claude too, via `runner/lib/compose.js`. Claude is the only LLM for generated prose and judgement, with no fallback backend. One narrow exception: 0-3 relevance **scoring** may run on a local `phi4-mini` via Ollama (`runner/lib/local_llm.js`, opt-in with `LOCAL_LLM_ENABLED=1`) — bounded classification, explicitly routed, never escalating to Claude. See INVENTORY §2.
