...
- **LinkedIn feed candidates now come from the voyager API, not the DOM** — the rendered feed only ever held **3** post cards: `body` is `overflow:hidden` and the document does not scroll, so every `window.scrollBy` in the engine was a silent no-op (fixed — see below), but even with real scrolling the feed reaches its *footer* after 3 cards and `&page=N` is ignored. The same account's `updatesV2?q=feed` returns **26** posts with full bodies. `fetchFeedCandidates()` reads that; `openPost(permalink)` then opens a target and stamps `data-hs-idx` so the existing like/comment code works unchanged. DOM `scrapeFeed` remains the fallback. Live-verified: 26 candidates → 3 scored ≥2 → 3 likes + 1 comment (dry run).
- **All nine `window.scrollBy`/`scrollTo` calls in the LinkedIn engine were no-ops** — LinkedIn scrolls an inner container (`<main>`), not the document. Replaced with `_scrollBy`/`_scrollTop`, which locate the real scroller **by geometry, not class name** (the markup is obfuscated). Verified: `scrollTop` 0 → 1242 → 1745 where it was previously pinned at 0.
- **Permalink pages need different selectors than feed cards** — a permalink has **zero** `div[role=listitem]`; the post is `div[role=article]`, and its like button is exactly `"React Like"` rather than `"Reaction button state …"`. `like()` accepts both, and the exact match keeps the per-comment `"React Like to <name>'s comment"` buttons out of range. `openPost` polls for hydration (a fixed wait raced it ~1 in 3).
- **Relevance scoring timed out on every post — LinkedIn engagement dead again, 2026-07-05 → 2026-08-08** (`runner/lib/content_relevance.js`, `runner/linkedin_engage.js`). Same symptom as the `await` bug below, different cause, and it started the same week the previous fix landed. The scorer's 30s timeout dates from the retired local brain; scoring now spawns a `claude -p` subprocess per post (~5s warm), and `engage()` scores the batch with `Promise.all` — so a 24–26-post LinkedIn feed launched ~25 concurrent spawns and *every* call blew 30s (measured on the runner host: 1 call 5.2s, 8 concurrent 27.6s; 206 such timeouts in one runner.log). `catch { return 0; }` then turned each timeout into "irrelevant", so nothing cleared min-2 and the run logged a healthy-looking `0 relevant … 0 like(s), 0 comment(s)`. Fixed: one process-wide semaphore (`RELEVANCE_CONCURRENCY`, 4) + `RELEVANCE_TIMEOUT_MS` (90s); failures return **-1 (skip)** and increment `scorer.stats.failed`, which `linkedin_engage`/`x_engage`/`x_amplify` log as a `WARN`. Live-verified dry run: 24 candidates → 2 relevant → 2 likes + 1 comment, 0 scoring failures, 36s wall clock. `x_engage` shared the bug and survived only on batch size (~9 posts ≈ 27.6s, one slow cycle from the same fate); it now shares the pool. Regression-tested in `runner/tests/run_tests.js` → "LinkedIn engagement wiring" (cases 5–6).
- **`LinkedIn.engage()` never awaited the relevance scorer** (`tools/helmstack-social/src/linkedin.js`) — when `linkedin_engage` swapped its synchronous keyword scorer for the async LLM scorer (2026-07-05, the helmstack-social extraction), `engage()` still did `score: score(p)` without `await`. `p.score` was a Promise, `Promise >= minScore` is always `false`, so `ranked` was silently always empty: every run since logged `0 relevant … done — 0 like(s), 0 comment(s)`, and `state/posts_log.json` records no `linkedin_like`/`linkedin_comment` after 2026-07-05. Now `await Promise.all(posts.map(async …))`, matching the X engine (`x.js`), which had the `await` all along. `linkedin_collect` was unaffected — it scores in its own awaited loop rather than through `engage()`.
