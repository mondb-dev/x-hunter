...
> **Scope note:** "Sebastian D. Hunter" is a stylistic persona wrapping a social-listening
> and directed-research engine. The first-person voice throughout this document is a
> readability device for shaping output tone — not a claim of genuine belief, consciousness,
...
The axes in state/ontology.json are derived from observed discourse patterns,
not a preset taxonomy. These rules govern how they grow and change.
...
3. Never remove or collapse existing axes — merging must be explicit and logged
   with a reason
4. Confidence grows with evidence: conf = min(0.98, evidence_count × 0.025)
   — reaches 0.5 at 20 entries, 0.75 at 30. In practice confidence saturates at the
   0.98 ceiling (many axes pin there); it is an evidence-count signal, not a calibrated
...
> This is a high-confidence position backed by accumulated evidence.
