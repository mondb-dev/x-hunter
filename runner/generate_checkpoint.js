...
`;
    ...
    // Shared 24h-gated reflection: daily maintenance runs it every day,
    // and checkpoints can still trigger it when run standalone.
    try {
      const reflection = await runProcessReflection({
        today,
        source: `checkpoint_${n}`,
        checkpointNumber: n,
        reportsLimit: 3,
      });
      if (reflection.status === "completed" && reflection.proposalTitle) {
        console.log(`[checkpoint] process proposal written: ${reflection.proposalTitle}`);
      } else {
        console.log(`[checkpoint] process reflection ${reflection.status}: ${reflection.reason}`);
      }
    } catch (err) {
