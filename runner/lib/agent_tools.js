...
const TOOL_EXECUTORS = {
  ...
  async track_narrative_propagation(args, ctx) {
    ...
    try {
      ...
      if (!posts || posts.length === 0) return `No new posts found for narrative "${narrative_id}".`;
      const newSightings = posts.slice(0, 20);
      ...
      narrative.metrics.reach_estimate = (narrative.metrics.reach_estimate || 0) + (posts.length * 1000);
      const authors = {};
      posts.forEach(p => { if(p.author_handle) { authors[p.author_handle] = (authors[p.author_handle] || 0) + 1; } });
      Object.entries(authors).forEach(([handle, postCount]) => {
        const existing = narrative.amplifiers.find(a => a.handle === handle);
        if (existing) { existing.posts_tracked += postCount; }
        else { narrative.amplifiers.push({ handle, posts_tracked: postCount, followers: 'unknown' }); }
      });
      narrative.amplifiers.sort((a, b) => b.posts_tracked - a.posts_tracked);
      narrative.amplifiers = narrative.amplifiers.slice(0, 20);
