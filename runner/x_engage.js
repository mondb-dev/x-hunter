...
async function generateReply(post) {
  ...
  try {
    ...
    // Shared outbound gate: voice_filter + fact-check + coherence. Passing
    // `source` turns on the coherence check; `regenerate` lets a misread be
    // re-drafted with the specific correction instead of dropped.
    const gated = await passOutbound(raw, {
      gates: ["voice", "factcheck"],
      maxLen: 270,
      tag: "x_reply",
      source: post.text || "",
      regenerate: async (why) => compose(
        prompt +
        `\n\nYOUR PREVIOUS DRAFT MISREAD THE POST — ${why}. Re-read the post above carefully: get WHO did WHAT to WHOM and the direction of events right, and do not invent a contradiction it doesn't contain. Write a corrected reply, or return SKIP if there is nothing accurate worth adding.`,
        { maxTokens: 400, model: "gemini-2.5-flash", thinkingBudget: 0, tag: "x_reply" }
      ),
    });
